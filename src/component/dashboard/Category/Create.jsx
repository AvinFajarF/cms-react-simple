...
export default function CategoryCreate() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8000/api/v1/category', {
        name: name,
      ...
      navigate('/category'); //ke categories
    })
    } catch (error) {
      console.error(error);
  ...
  return (
    <div>
      <Container className="mt-3">
        <Button href="/category" variant="dark" className="mb-3">Kembali ke category</Button>
        <h2>Buat Category</h2>
