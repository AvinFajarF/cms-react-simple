...
function Categories() {
    ...
    useEffect(() => {
      axios.get("http://localhost:8000/sanctum/csrf-cookie").then(() => {
        axios
        .get('http://localhost:8000/api/v1/category',{headers:{Authorization: `Bearer ${token}`}})
        .then((response) => {
...
const handleDeletePost = async (categoryId) => {
    try {
      axios.get("http://localhost:8000/sanctum/csrf-cookie").then(async () => {
      await axios.delete(`http://localhost:8000/api/v1/category/delete/${categoryId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    return (
        ...
        <Container className="mt-3">
          <Button href="/category/create" variant="dark" className="mb-3">
            Create Category
          </Button>
